...
const updateProfile = async(req, res) => {
    try {
        let { name, email, companyName, position, city } = req.body;
        email = email.toLowerCase()
        const { id } = req.params;
        if (id == req.user._id) {
            if (email == req.user.email) {
                const data = await User.findByIdAndUpdate({ _id: id }, { name, companyName, position, city }, { new: true });
                ...
                if (emailExist) {
                    ...
                    const data = await User.findByIdAndUpdate({ _id: id }, { name, email, companyName, position, city, verified: false, logedIn: false, verificationKey }, { new: true });
