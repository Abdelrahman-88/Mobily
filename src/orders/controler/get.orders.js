...
const getOrderById = async(req, res) => {
    try {
        ...
        let order = await Order.findOne({ _id: orderId }).populate({ path: "cart priceOffer", populate: { path: "services.serviceId" } }).populate({ path: "createdBy", populate: { path: "documentId" }, select: "-password -verificationKey" })
        if (order) {
            if (req.user._id.equals(order.createdBy._id)) {
                const seen = await Order.findOneAndUpdate({ _id: orderId }, { seen: true }, { new: true }).populate({ path: "createdBy", populate: { path: "documentId" }, select: "-password -verificationKey" }).populate({ path: "cart priceOffer", populate: { path: "services.serviceId" } })
                res.status(StatusCodes.OK).json({ message: "Done", data: seen });
            } else if (req.user.role == "operator") {
                if (order.action) {
                    order = await Order.findOne({ _id: orderId }).populate({ path: "createdBy", populate: { path: "documentId" }, select: "-password -verificationKey" }).populate({ path: "cart priceOffer", populate: { path: "services.serviceId" } }).populate("actionBy", "employeeId")
                    ...
                    const action = await Order.findOneAndUpdate({ _id: orderId, status: { $ne: "closed" } }, { actionBy: req.user._id, action: true }, { new: true }).populate({ path: "createdBy", populate: { path: "documentId" }, select: "-password -verificationKey" }).populate("actionBy", "employeeId").populate({ path: "cart priceOffer", populate: { path: "services.serviceId" } })
                    if (action) {
...
const getAllOrders = async(req, res) => {
    try {
        ...
        const orders = await searchServies("", { status, activated, action, actionBy, type }, limit, skip, Order, [], "cart priceOffer createdBy", "companyName", "")
...
const getUserOrders = async(req, res) => {
    try {
        ...
        if (req.user._id == createdBy) {
            ...
            const { skip, limit, currentPage } = pageService(page, size)
            const orders = await searchServies("", { createdBy, status, activated, type }, limit, skip, Order, [], "cart priceOffer", "", "")
            if (orders.data.length) {
                res.status(StatusCodes.OK).json({ message: "done", currentPage, limit, totalPages: orders.totalPages, total: orders.total, data: orders.data });
            } else {
                res.status(StatusCodes.BAD_REQUEST).json({ message: "No orders found" });
